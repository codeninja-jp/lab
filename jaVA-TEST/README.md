...
```bash
jaVA-TEST$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
jaVA-TEST$ sam local start-api
jaVA-TEST$ curl http://localhost:3000/
```
...
```bash
jaVA-TEST$ sam logs -n HelloWorldFunction --stack-name jaVA-TEST --tail
```
...
```bash
jaVA-TEST$ cd HelloWorldFunction
...
```bash
aws cloudformation delete-stack --stack-name jaVA-TEST
```
